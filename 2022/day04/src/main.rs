...
fn main() {
    ...
    println!("Part A result: {}", count_overlaps(input.to_string(), OverlapType::Completely));
    println!("Part B result: {}", count_overlaps(input.to_string(), OverlapType::Partial));
}
...
fn count_overlaps(input:String, overlap_type:OverlapType) -> usize {
    return input.split("\n")
